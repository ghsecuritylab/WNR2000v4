...
#ifdef CONFIG_PPP_TM_ACCURATE_CONTROL
unsigned long long ppp_tm_limit = 0xffffffffffffffff;
int ppp_need_back;
int ppp_tm_dir;
...
struct ppp_msg_data{
        ...
        int tm_dir; /*1--both,0--only download,2--abolish*/
...
void ppp_test_netlink(struct sk_buff *__skb)
{
	 ...
	 if(msgdata->tm_dir == 2)
	 	ppp_tm_limit = 0xffffffffffffffff;
	 else 
	 ...
	 if(msgdata->tm_dir == 3)
	 	...
	 	need_drop = 0;
         kfree_skb(skb);
         printk("PPP Left volume is %llu dir is %d ...\n",msgdata->leftdata,ppp_tm_dir);
...
static int __init ppp_init(void)
{
	int err;
       ...
       ppp_nl_sk = netlink_kernel_create(&init_net,PPP_NETLINK_TM,1,ppp_test_netlink,NULL,THIS_MODULE);
        if(ppp_nl_sk < 0){
                printk("netlink_ppp_driver_create error ...\n");
        }
#endif
...
#ifdef CONFIG_PPP_TM_ACCURATE_CONTROL
        if(ppp_tm_limit != 0xffffffffffffffff)
