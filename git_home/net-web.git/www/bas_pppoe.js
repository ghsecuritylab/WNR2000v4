...
function check_pppoe(form,check)
{
		...
		if(form.pppoe_myip.value !="...")
		{
			form.intranet_wan_assign.value = 1;
                	...
                	if(isSameIp(form.pppoe_myip.value,lan_ip) == true)
                	{
                        ...
                        if(checksubnet(form.pppoe_mask.value, 1)==false)//35757 change flag to 1
                        {
                                alert("$invalid_mask");
                                return false;
                        }
		...
		if(russian_pppoe_flag == 1 && gui_region == "Russian")
                {
                	...
                	if((form.pppoe_subnet.value != "...")&&(form.pppoe_subnet.value != "0.0.0.0")) 
                        {
				if (checksubnet(form.pppoe_subnet.value, 1)==false)//35757 change flag to 1
        			{
	        			alert("$invalid_mask");
	        			return false;
        			}
	...
	//to fix bug31737
	if( form.conflict_wanlan.value == 1 )
	{
		var oldNetwork = lan_subnet.split(".");
		var new_subnet="255.255.255.0";
		var nowNetwork = new_subnet.split(".");
		oldNumofNetwork = getNumOfNetwork(oldNetwork);
		newNumofNetwork = getNumOfNetwork(nowNetwork);
		if ( oldNumofNetwork < newNumofNetwork ) //If subnet is changed and the subnet range is from big to small, router SHOULD flush dmz.
		{
			form.flush_flag.value=1;
		}
	}
