...
//.o
	...
	var str = eval('node' + id).replace(/&nbsp;/g," ").replace(/&#92;/g, "\\").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&#40;/g,"(").replace(/&#41;/g,")").replace(/&#34;/g,'\"').replace(/&#39;/g,"'").replace(/&#35;/g,"#").replace(/&#38;/g,"&");
	echo_info = str.split('*');
       ...
       if(echo_info[0]=='0') // volume name could have '*', volume name save in nodevol, so disk node's value less a item than normal notes.
        {
                ...
                var value=""+echo_info[0]+"*"+cn.io+"*"+echo_info[2]+"*"+echo_info[3];
        }
        else
        {
