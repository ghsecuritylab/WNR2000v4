...
function Check_add(cf)     
{
	...
	for(i=1;i<=forward_array_num;i++)
	{
		...
		{
			if (!(parseInt(ext_endport)<parseInt(input_external_start_port)||parseInt(input_external_end_port)<parseInt(ext_startport))
				&& (forwardingip != input_ip || parseInt(int_startport) != (parseInt(input_internal_start_port)+parseInt(ext_startport)-parseInt(input_external_start_port))))
 			{
...
function check_forwarding_add(cf,flag)
{
	...
	if(check_port_range(str_exter_port)==false)
	{
	...
	else
	{
		...
		if(check_port_range(str_internal_port)==false)
		{
	...
	for(k=0;k<input_ext_start_port.length-1;k++)
	{
		...
		for(i=1;i<=forward_array_num;i++)
		{
			...
			else if (sertype=="TCP/UDP"||sertype==input_sertype||input_sertype=="TCP/UDP")
			{
				if(flag == 'edit')
				{
					if (!(parseInt(ext_endport)<parseInt(input_ext_start_port[k])||parseInt(input_ext_end_port[k])<parseInt(ext_startport)) && !(select_editnum<=i && i<parseInt(select_editnum)+parseInt(edit_num))
						&& (input_ip != forwardingip || parseInt(int_startport) != (parseInt(input_int_start_port[k])+parseInt(ext_startport)-parseInt(input_ext_start_port[k]))))
					{
						alert("$ports_error_conflict");
						return false;
					}	
					...
					if (!(parseInt(ext_endport)<parseInt(input_ext_start_port[k])||parseInt(input_ext_end_port[k])<parseInt(ext_startport))
						&& (forwardingip != input_ip || parseInt(int_startport) != (parseInt(input_int_start_port[k])+parseInt(ext_startport)-parseInt(input_ext_start_port[k]))))
					{
							alert("$ports_error_conflict");
							return false;
					}
		...
		for(i=1;i<=upnp_array_num;i++)
		{
			...
			upnp_ext=each_info[3];
			upnp_ip=each_info[4];
			if (!( parseInt(upnp_ext)<parseInt(input_ext_start_port[k]) || parseInt(input_ext_end_port[k])<parseInt(upnp_int) ) || !( parseInt(upnp_ext)<parseInt(input_int_start_port[k]) || parseInt(input_int_end_port[k])<parseInt(upnp_int) ))
			{
	...
	if(flag=='edit')
		cf.submit_flag.value="forwarding_edit_range";
	else
		cf.submit_flag.value="forwarding_add_range";
	
	return true;
}

function int_port_value_range()
{
	var cf=document.forms[0];
	if(cf.same_range.checked == true)
		cf.internal_port.value=cf.exter_port.value;
}
function arrange_by_ip(type)
{
        if(forward_array_num>0)
        {
                ...
                if(type==1)
                        ...
                        trValue.sort(compareCols(7,"int"));
