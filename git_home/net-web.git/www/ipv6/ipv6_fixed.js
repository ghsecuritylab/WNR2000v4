function check_ipv6_fixed(cf)
{
	...
	for( i=0; i<cf.IPv6_wan.length; i++ )
	{
		...
		if( check_pri_dns ){// to fix bug 26923
			...
			if(cf.IPv6_Pri_DNS[i].value == "")
			{
				...
				cf.ipv6_primary_dns_fixed.value = cf.ipv6_primary_dns_fixed.value + "0" + ":";/* to fix bug 24430 , bug35472 */
			}
			else
			{
		...
		if( check_sec_dns ){// to fix bug 26923
			...
			if(cf.IPv6_Sec_DNS[i].value == "")
			{
				...
				cf.ipv6_second_dns_fixed.value = cf.ipv6_second_dns_fixed.value + "0" +  ":";/* to fix bug 24430 , bug35472 */
			}
			else
			{
	...
	{
		alert("$invalid_wan_ipv6");//to fix bug34063
		return false;
	}
	if(lan_ip!="" && (lan_ip==primary_dns||lan_ip==second_dns))
	{
		alert("$invalid_lan_ipv6");//to fix bug34063
		return false;
	}
	/***********************************************/
