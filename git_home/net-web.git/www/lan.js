...
function check_clientNumber(cf)
{
	...
	var localip = parseInt(lan_array[3],10);//to fix bug31265
...
*/
	...
	if( wan_ip!="0.0.0.0" )
	{
		if( wan_type == "pppoe" || wan_type == "pptp" || wan_type == "mulpppoe1" )
		{
			...
			{
				alert("$conflict_with_wanip");//to fix bug30965,change conflicted_with_wanip to  conflict_with_wanip
				return false;
			}
		}
		else
	...
	//to fix bug30838:set LAN ip address the same to My ip,it should conflict and pop up warning message
	if(pppoe_intranet_flag==1)
	{
		if(isSameSubNet(form.lan_ipaddr.value,form.lan_subnet.value,wan_intr_ip,form.lan_subnet.value) == true )
		{
			alert("$conflict_with_wanip");
			return false;
		}
		if(isSameIp(wan_intr_ip,form.lan_ipaddr.value) == true)
		{
			alert("$conflict_with_wanip");
			return false;
		}
...
*/
		alert("$change_ip_manually1");//to fix bug30511

		if(isSameSubNet(form.lan_ipaddr.value,"255.255.255.0",old_lanip,"255.255.255.0")==false) //33654 only keep the last octet
		{		
			//lan subnet
...
/* 33654
			...
			if(new_lansubnet_array[1] != "0" && new_lansubnet_array[2] == "0" && new_lansubnet_array[3] == "0")
			{
				sub_leng=2;
			...
			else if(new_lansubnet_array[0] != "0" && new_lansubnet_array[1] == "0" && new_lansubnet_array[2] == "0" && new_lansubnet_array[3] == "0")
			{
				sub_leng=1;
