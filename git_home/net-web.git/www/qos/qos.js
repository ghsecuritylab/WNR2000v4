...
function change_serv_adva()
{
	...
	else
	{
		...
		if(serv_array[i][3]=="IP_Phone")
			cf.name.value="IP Phone (port 6670, includes SIP & H.323 IP phones)";
			...
			cf.name.value="Netmeeting (port 1720)";
		else
...
function change_name_online(name)
{
		if(name=="Counter-Strike")
			...
			new_name="IP Phone (port 6670, includes SIP & H.323 IP phones)";
...
function check_qos_apply(cf)
{
	...
	else if(cf.bw_sel[1].checked == true)
	{
		...
		qos_bandwith=cf.qos_auto_bandwidth.value;
	}
	if ( cf.turn_qos_bandwidth_on.checked == true )
	{
				...
				if(qos_bandwith == 0 || qos_bandwith == "")
				{
					...
					else
					{
						if(!(qos_bandwith>0&&qos_bandwith<1001))
						{
							...
							cf.qos_hidden_uprate.value = tcbw_value*1024;
					}
				}
				else
				{
					if(!(qos_bandwith>0&&qos_bandwith<1024001))
					{
						...
						cf.qos_hidden_uprate.value = parseInt(cf.qos_auto_bandwidth.value);
				}
			}
		}
		else if (max_bandwidth == 100)
		{
			...
			else if(cf.bw_sel[1].checked == true)
			{
				if(qos_bandwith == 0 || qos_bandwith == "")
				{
					qos_bandwith=tcbw_value;//to fix bug35326
					if(cf.tcbw_unit.selectedIndex == 0)
					{
						if(!(qos_bandwith>0&&qos_bandwith<102401))
						{
