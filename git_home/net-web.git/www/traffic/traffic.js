...
function set_gray()
{
	...
	document.getElementById("refresh").className  = "common_big_bt";
	document.getElementById("status").className  = "common_big_bt";	
	change_volumn_time();
	}
	else
	{
	document.getElementById("restart").className  = "long_common_gray_bt";
	document.getElementById("refresh").className  = "common_big_gray_bt";
	document.getElementById("status").className  = "common_big_gray_bt";
	}
...
function check_traffic_apply(cf)
{
	...
	if(cf.checkTraffic.checked == true)
	{
		...
		if(cf.tm_type[0].checked)
		{
			...
			if(parseInt(cf.waterMark.value) != "0" || parseInt(cf.volume_monthly_limit.value) !="0")
			{
				if(parseInt(cf.waterMark.value) >= parseInt(cf.volume_monthly_limit.value))//to fix bug30466
				{
			...
			if(parseInt(cf.round_up_volume.value)!="0" || parseInt(cf.volume_monthly_limit.value)!="0")
			{
				if(parseInt(cf.round_up_volume.value) >= parseInt(cf.volume_monthly_limit.value))//fix bug30465
				{
		...
		else
		{
			...
			if(parseInt(cf.waterMark.value) != "0" || parseInt(cf.conntime_monthly_limit.value) != "0")
			{
				if(parseInt(cf.waterMark.value) >= parseInt(cf.conntime_monthly_limit.value)*60)//to fix bug30466
				{
					alert("$left_volumn_small");
					return false;
				}
...
function reset_time()
{
	...
	//to fix bug30463:The range of 08000-09999 seconds should be accepted
