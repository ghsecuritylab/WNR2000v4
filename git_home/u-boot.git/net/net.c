...
NetLoop(proto_t protocol)
{
	...
	eth_halt();
#ifdef CONFIG_NET_MULTI
#if defined(CFG_VITESSE_73XX_NOPHY) || defined(CONFIG_WNR1000V2) || defined(CONFIG_WNR1100) || defined(CONFIG_WNR2200) || defined(CONFIG_WNR2000V3) || defined(CONFIG_WNR1000V4) || defined(CONFIG_HW29763904P4P32) || defined(CONFIG_HW29763906P8P0P64P2x2P0)
	/*
...
void update_firmware(ulong addr, int firmware_size)
{
	...
	update_data(addr, firmware_size, CFG_IMAGE_ADDR_BEGIN, CFG_IMAGE_LEN +
			board_image_reserved_length(), 1);
#ifdef CFG_NMRP
	if(NmrpState != 0)
		return;
#endif
...
tftpstart:
	...
	if ((!board_model_id_match_open_source_id() && !image_match_open_source_fw_id(addr)) &&
	...
	if (!board_model_id_match_open_source_id() && (!board_match_image_model_id(addr) &&
