...
static int DHCP_Detection(int *bpain, uint8 *arp)
{
	int i;
	int fd, ret, fond;
	uint32 xid;
...
uint32_t do_detection(char *dutMac, char *pcMac)
{
	int bpa;
...
static int checkType(uint32_t ConnType)
{
	...
	if (ConnType & BPAFound) {
		...
		sprintf(cmd, "/sbin/udhcpc -b -i %s -h %s", wan_if_name, host_name);
		system(cmd);
