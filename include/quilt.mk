...
define PatchGenericDir/Quilt
	@if [ -s "$(2)/series" -a "$(3)" != "dni" ]; then \
		mkdir -p "$(1)/patches/generic/"; \
		cp "$(2)/series" "$(1)/patches/generic/"; \
	fi
	@for patch in $$$$( (cd "$(2)" && if [ -f series ]; then $(call filter_series,series); else ls; fi; ) 2>/dev/null ); do ( \
		if [ "$(3)" = "dni" ]; then continue; fi; \
		cp "$(2)/$$$$patch" "$(1)"; \
		cd "$(1)"; \
		$(QUILT_CMD) import -Pgeneric/$$$$patch -p 1 "$$$$patch"; \
		$(QUILT_CMD) push -f >/dev/null 2>/dev/null; \
		rm -f "$$$$patch"; \
	); done
	@echo generic/ >> "$(1)/patches/.subdirs"
endef

define PatchGenericDir/Default
	if [ -d "$(2)" -a "$$$$(ls $(2) | wc -l)" -gt 0 -a "$(3)" != "dni" -a "$(3)" != "netgear" ]; then \
		if [ -s "$(2)/series" ]; then \
...
define Build/PatchEnabled/Default
	@if [ "$(patches-y)" != "" ]; then \
		for patch in $(patches-y); do \
			if [ ! -f "$$$$patch" ]; then \
				echo "Failed: $$$$patch does not exist"; exit 1; \
			fi; \
		done; \
		TMP_PATCH_DIR=`mktemp -d -p ./ -t patches.XXXXXXXXXX`; \
		$(CP) $(patches-y) $$$$TMP_PATCH_DIR; \
		$(PATCH) $(PKG_BUILD_DIR) $$$$TMP_PATCH_DIR; \
		rm -rf $$$$TMP_PATCH_DIR; \
	fi
endef

define PatchGenericDir
$(call PatchGenericDir/$(if $(strip $(QUILT)),Quilt,Default),$(strip $(1)),$(strip $(2)),$(strip $(3)))
endef
