...
if [ "\$(config get wan_proto)" = "pptp" -a "\$(config get wan_pptp_wan_assign)" = "1" ]; then
	pptp_gw="\$(config get pptp_gw_static_route)"
	pptp_mask="\$(config get wan_pptp_eth_mask)"
	if [ "x\$staticdns1" != "x" ]; then
		echo nameserver "\$staticdns1" >> /tmp/resolv.conf
	        if [ "x\$(config get pptp_gw_static_route)" != "x" ] && compare_the_same_subnet "\$pptp_gw" "\$staticdns1" "\$pptp_mask"; then
			/sbin/route -A inet add -net "\$staticdns1" netmask 255.255.255.255 gw "\$(config get pptp_gw_static_route)"  br1
		fi
	fi
	if [ "x\$staticdns2" != "x" ];then
		echo nameserver "\$staticdns2" >> /tmp/resolv.conf
		if [ "x\$(config get pptp_gw_static_route)" != "x" ] && compare_the_same_subnet "\$pptp_gw" "\$staticdns2" "\$pptp_mask"; then
			/sbin/route -A inet add -net "\$staticdns2" netmask 255.255.255.255 gw "\$(config get pptp_gw_static_route)"  br1
		fi
	fi
elif [ "\$(config get wan_proto)" = "l2tp" -a "\$(config get wan_l2tp_wan_assign)" = "1" ]; then
        l2tp_gw="\$(config get l2tp_gw_static_route)"
        l2tp_mask="\$(config get wan_l2tp_eth_mask)"
	if [ "x\$staticdns1" != "x" ]; then
		echo nameserver "\$staticdns1" >> /tmp/resolv.conf
		if [ "x\$(config get l2tp_gw_static_route)" != "x" ] && compare_the_same_subnet "\$l2tp_gw" "\$staticdns1" "\$l2tp_mask"; then
			/sbin/route -A inet add -net "\$staticdns1" netmask 255.255.255.255 gw "\$(config get l2tp_gw_static_route)" br1
		fi
	fi
	if [ "x\$staticdns2" != "x" ];then
		echo nameserver "\$staticdns2" >> /tmp/resolv.conf
		if [ "x\$(config get l2tp_gw_static_route)" != "x" ] && compare_the_same_subnet "\$l2tp_gw" "\$staticdns2" "\$l2tp_mask"; then
			/sbin/route -A inet add -net "\$staticdns2" netmask 255.255.255.255 gw "\$(config get l2tp_gw_static_route)"  br1
		fi
	fi
fi
...
if [ -f /tmp/ru_static_route ]; then
	cat /dev/null > /tmp/ru_static_route
fi
if [ -f /tmp/ru_l2tp_static_route ]; then
	cat /dev/null > /tmp/ru_l2tp_static_route
fi
EOF
}
...
setup_interface_ppp() {
	...
	if [ "$($CONFIG get wan_pptp_wan_assign)" = "0" -a "$($CONFIG get wan_pptp_dns_assign)" = "0" -o "$($CONFIG get wan_pptp_wan_assign)" = "1" ]; then
		dns="usepeerdns"
	fi
	...
	language="language $($CONFIG get GUI_Region)"
        ...
        if [ "$($CONFIG get wan_pptp_dns_assign)" != "0" -a "$($CONFIG get wan_pptp_wan_assign)" = "1" ]; then
                staticdns1="$($CONFIG get wan_ether_dns1)"
                staticdns2="$($CONFIG get wan_ether_dns2)"
	...
	if [ "$($CONFIG get wan_pptp_dns_assign)" = "1" -a "$($CONFIG get wan_pptp_wan_assign)" = "1" ]; then
	...
	if [ "$($CONFIG get wan_l2tp_wan_assign)" = "0" -a "$($CONFIG get wan_l2tp_dns_assign)" = "0" -o "$($CONFIG get wan_l2tp_wan_assign)" = "1" ]; then
		dns="usepeerdns"
	fi
        if [ "$($CONFIG get wan_l2tp_dns_assign)" != "0" -a "$($CONFIG get wan_l2tp_wan_assign)" = "1" ]; then
            staticdns1="$($CONFIG get wan_ether_dns1)"
            staticdns2="$($CONFIG get wan_ether_dns2)"
        ...
        if [ "$($CONFIG get wan_l2tp_dns_assign)" = "1" -a "$($CONFIG get wan_l2tp_wan_assign)" = "1" ]; then
            echo "$staticdns1" >> /etc/ppp/peers/dial-provider
            echo "$staticdns2" >> /etc/ppp/peers/dial-provider
        fi
