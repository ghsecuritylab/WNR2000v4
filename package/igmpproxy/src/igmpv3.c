...
add_ebt_rule(uint32 port, uint32 group, uint32 newmode, struct source_set *newset)
{
	...
	if (newset)
	{
		len = sprintf(line, "ebtables -I OUTPUT -o eth1 -p ipv4 --ip-dst %u.%u.%u.%u ", 
			NIPQUAD(group));
		if (num)
...
del_ebt_rule(uint32 port, uint32 group, uint32 oldmode, struct source_set *oldset)
{
	...
	if (oldset)
	{
		len = sprintf(line, "ebtables -D OUTPUT -o eth1 -p ipv4 --ip-dst %u.%u.%u.%u ", 
...
{
	int i;
	if (mc_ctrl_packet(croute->group))
		return;
	...
	for (i = 0;(i < num) && pdata; i++, pdata += grec->grec_nsrcs*4 + 8)
	{
		...
		else
		{
			...
			else
			{
				...
				if (mode == st_is_in && !memcmp(&tmp1, &ZEROSET, sizeof(struct source_set)))
					acceptLeaveMessage(src, group);
				else
