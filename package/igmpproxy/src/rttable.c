...
void logRouteTable(char *header) {
    IF_DEBUG  {
        struct RouteTable*  croute = routing_table;
	struct Listener*    clistener = NULL;
        unsigned            rcount = 0;
            ...
            do {
                ...
                */
                igmp_syslog(LOG_DEBUG, 0, "#%d: Src: %s, Dst: %s, Age:%d, St: %s, OutVifs: 0x%08x",
                    ...
                    croute->vifBits);
		if(croute->listeners != NULL){
			clistener = croute->listeners;
			do{
				igmp_syslog(LOG_DEBUG, 0, "####### listeners: %s, port: %d, time:%d, mode:%d",inetFmt(clistener->srcAddr, s1),clistener->port,
					(GetSysUpTime() - clistener->timeout),clistener->mode);
				clistener = clistener->nextlistener;
			} while (clistener != NULL );
		}
		else
			igmp_syslog(LOG_DEBUG, 0, "####### listeners: NULL");
