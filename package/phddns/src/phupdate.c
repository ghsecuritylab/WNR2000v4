...
int phddns_step(PHGlobal *phglobal)
{
	...
	if (phglobal->bNeed_connect)
	{
		...
		if (!InitializeSockets(phglobal))
		{
			...
			//if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(4, -1);
			return 5;
		}
		...
		if (ret == okDomainsRegistered) 
		{
			...
			if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(1,phglobal->nUserType);
		...
		else 
		{
			...
			{
				if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(3,-1);
